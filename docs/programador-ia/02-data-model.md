...
- El motor lee `Programaciones` existentes (confirmadas, `trabajador_id IS NOT NULL`) para calcular la carga preexistente del técnico ese día.
- Al confirmar, el módulo siempre hace `INSERT` de una nueva `Programacion` con `trabajador_id` ya asignado.
- El campo `descripcion` se usa para registrar la justificación del LLM.
...
## Joins necesarios para enriquecer el trabajo del formulario

Cuando el admin envía `{ cliente_id, ascensor_id, tipo_trabajo }`, el backend enriquece con:

| Campo | Tabla origen |
|---|---|
| `nombre_comercial` | `Clientes` |
| `distrito` | `Clientes` |
| `latitud`, `longitud` | `Clientes` |
| `tipo_equipo` | `Ascensores` |
| `marca` | `Ascensores` |
| `duracion_min` | `ConfiguracionIA` (por `tipo_trabajo`) |
| `prioridad` | `ConfiguracionIA` (por `tipo_trabajo`) |

Join: `Ascensores JOIN Clientes ON Clientes.cliente_id = Ascensores.cliente_id`

Ver implementación en `03-demand-sources.md`.
